#!/usr/bin/env python3
"""
Agent Config Generator - Creates agent-specific config files for a skill folder.

Usage:
    generate_agent_config.py <skill_dir> [--agent <agent_name>] [--name <skill_name>] [--interface key=value]
"""
...
def write_agent_config(skill_dir, skill_name, agent_name, raw_overrides):
    """
    ...
    Args:
        ...
        agent_name: Name of the agent platform (e.g., 'openai', 'anthropic', 'generic')
        raw_overrides: List of interface overrides in key=value format

    Returns:
        Path to created config file, or None if error
    """
    ...
    # Use agent-specific filename
    output_path = agents_dir / f"{agent_name}.yaml"
    output_path.write_text("\n".join(interface_lines) + "\n")
    print(f"[OK] Created agents/{agent_name}.yaml")
    return output_path


def main():
    parser = argparse.ArgumentParser(
        description="Create agent-specific config file for a skill directory.",
    )
    parser.add_argument("skill_dir", help="Path to the skill directory")
    parser.add_argument(
        "--agent",
        default="openai",
        help="Agent platform name (default: openai). Creates agents/<agent>.yaml",
    )
    parser.add_argument(
        "--name",
    ...
    result = write_agent_config(skill_dir, skill_name, args.agent, args.interface)
    if result:
        sys.exit(0)
    sys.exit(1)
