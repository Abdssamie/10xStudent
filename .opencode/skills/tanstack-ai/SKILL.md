...
// Good
description: "Get user information from the database by user ID. Use this when you need to look up user details like name, email, or account status.";
```

### Issue: Type errors with tool parameters

**Solution:** Ensure Zod schema matches your implementation:
...
async ({ userId, includeMetadata }) => {
  // userId is string
  // includeMetadata is boolean | undefined
};
```

### Issue: Stream not closing

**Solution:** Ensure you're properly yielding all chunks:

```typescript
const trackedStream = (async function* () {
  for await (const chunk of stream) {
    yield chunk; // Don't forget to yield!
  }
