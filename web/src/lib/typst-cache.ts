...
export async function getCachedWasm(
  ...
  if (typeof caches === 'undefined') {
    log.warn(`${tag} Cache API unavailable - fetching fresh from ${url}`);
    return url;
  }
  ...
  try {
    ...
    log.log(`${tag} Looking up cache entry for key: "${key}"`);

    const cached = await cache.match(key);

    if (cached) {
      const blob = await cached.blob();
      log.log(`${tag} CACHE HIT - serving ${type} WASM from Cache API (${(blob.size / 1024).toFixed(1)} KB). No network request.`);
    ...
    log.log(`${tag} CACHE MISS - fetching fresh ${type} WASM from "${url}". Will be cached for next load.`);
    ...
    log.log(`${tag} Fetched in ${fetchMs} ms (${(bytes.byteLength / 1024).toFixed(1)} KB). Storing in cache.`);

    await cache.put(key, new Response(bytes.slice(0), {
      headers: { 'Content-Type': 'application/wasm' },
    }));
    log.log(`${tag} Stored in cache. Next load will be a cache hit.`);
    ...
    log.error(`${tag} Cache strategy failed - falling back to direct URL "${url}":`, error);
    return url;
  }
}

export async function clearWasmCache(): Promise<void> {
  log.log('[typst-cache] Clearing WASM cache:', CACHE_NAME);
  await caches.delete(CACHE_NAME);
  log.log('[typst-cache] Cache cleared.');
}
...
export function revokeWasmBlobUrls(): void {
  log.log(`[typst-cache] Revoking ${activeBlobUrls.size} blob URLs`);
